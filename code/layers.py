...
class SpGraphAttentionLayer(nn.Module):
    ...
    def __init__(self, num_nodes, num_relation, in_features, out_features, nrela_dim, dropout, alpha, concat=True,
                 rand_init=False):
        ...
        self.a_h = nn.Parameter(torch.zeros(size=(out_features, in_features)))
        nn.init.xavier_normal_(self.a_h.data, gain=1.414)
        self.a_r = nn.Parameter(torch.zeros(size=(out_features, nrela_dim)))
        nn.init.xavier_normal_(self.a_r, gain= 1.414)
...
#       edge_h = torch.cat((input[edge[0, :], :], input[edge[1, :], :], edge_embed[:, :]), dim=1).t()
        edge_head = self.a_h.mm((input[edge[0, :], :] + input[edge[1, :], :]).t())
        edge_relation = self.a_r.mm((edge_embed[:, :]).t())
        #edge_m = self.a.mm(edge_h)
        edge_m = edge_head * edge_relation
        # edge_m: D * E
        ...
        # edge_w: E * D
        del edge_head, edge_relation, edge_e, edge_m
